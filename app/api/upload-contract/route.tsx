...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // HWP 파일을 임시 경로에 저장
    const tempFilePath = path.join(os.tmpdir(), fileName); // 임시 파일 경로
    const buffer = Buffer.from(await file.arrayBuffer());
    fs.writeFileSync(tempFilePath, buffer);

    // HWP 파일을 PDF로 변환 (convertHwpToPdf API 호출)
    const res = await fetch(`http://localhost:3000/api/converttopdf?fileName=${fileName}`);
    const data = await res.json();
    if (data.error) {
      return NextResponse.json({ error: "Failed to convert HWP to PDF", details: data.error }, { status: 500 });
    }

    const pdfFilePath = data.pdfFilePath;

    // 변환된 PDF 파일을 Azure Blob Storage에 업로드
    const pdfBlobClient = containerClient.getBlockBlobClient(newFileName.replace(fileType, "pdf"));
    const pdfBuffer = fs.readFileSync(pdfFilePath);
    await pdfBlobClient.upload(pdfBuffer, pdfBuffer.length, {
      blobHTTPHeaders: { blobContentType: "application/pdf" },
    });

    console.log(`File ${newFileName} uploaded as PDF to Azure Blob Storage for user ${userId}`);

    // Insert file information into database
    await db.query(
      "INSERT INTO contract_postfile (con_id, con_filetype, con_datetype, con_filename) VALUES (?, ?, NOW(), ?)",
      [newContractId, "pdf", newFileName.replace(fileType, "pdf")]
    );

    // Clean up temporary files
    fs.unlinkSync(tempFilePath); // HWP 파일 삭제
    fs.unlinkSync(pdfFilePath); // 변환된 PDF 파일 삭제
